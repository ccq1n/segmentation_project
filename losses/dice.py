...
from networks.utils import one_hot
from utils.enums import LossReduction, Weight


class DiceLoss(_Loss):
    """
    Compute average Dice losses between two tensors. It can support both multi-classes and multi-labels tasks.
...
class MaskedDiceLoss(DiceLoss):
    ...
    This has the effect of ensuring only the masked region contributes to the losses computation and
...
class GeneralizedDiceLoss(_Loss):
    """
    Compute the generalised Dice losses defined in:
        Sudre, C. et. al. (2017) Generalised Dice overlap as a deep learning
        losses function for highly unbalanced segmentations. DLMIA 2017.
    Adapted from:
...
class GeneralizedWassersteinDiceLoss(_Loss):
    ...
    [2] "Generalised dice overlap as a deep learning losses function
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, smooth: float = 1e-5) -> torch.Tensor:
        ...
        # Compute and return the final losses
